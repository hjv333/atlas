...
public class LocalAtlasOMRSMetadataCollection extends OMRSMetadataCollection {
            ...
            UserNotAuthorizedException
    {
        ...
        // Check whether the type being sought is one of the FamousFve - if so map the name.
        if (FamousFive.omTypeRequiresSubstitution(name)) {
            // The type to be found is in the famous five.
            // We do not have the OM GUID but all we need at the moment is the Atlas type name to look up so GUID can be null
            name = FamousFive.getAtlasTypeName(name, null);
            LOG.debug("{} type name mapped to {}", methodName, name);
        }
    ...
    public List<AttributeTypeDef> findAttributeTypeDefsByCategory(String                   userId,
        ...
        LOG.error("==> findAttributeTypeDefsByCategory(userId={}, category={})", userId, category);
            ...
            UserNotAuthorizedException
    {
        ...
        LOG.error("==> _findAttributeTypeDefsByCategory(userId={}, category={})", userId, category);
    ...
    private AttributeTypeDef _getAttributeTypeDefByGUID(String userId,
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_GUID_NOT_FOUND) {
                LOG.debug("{}: Atlas does not have the type with guid {} ", methodName, guid, e);
                // The AttributeTypeDef was not found - return null
                abtd = null;
            } else {
                LOG.error("{}: caught exception trying to retrieve type with guid {}", methodName, guid, e);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
        ...
        // AtlasBaseTypeDef abtd is known to be good
            ...
            TypeDefNotKnownException
    {
        ...
        if (FamousFive.omTypeRequiresSubstitution(omTypeName)) {
            ...
            LOG.debug("_getTypeDefByName: name {} substituted to {}", omTypeName, atlasTypeName);
        }
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                ...
                String errorMessage = errorCode.getErrorMessageId()
                        ...
                        this.getClass().getName(),
                        "_getTypeDefByName",
                        e.getMessage(), metadataCollectionId);
        ...
        if (abtd == null) {
            ...
            LOG.error("_getTypeDefByName: received null return from Atlas getByName using name {}", atlasTypeName);
        ...
        TypeDef ret;
        try {
            LOG.debug("_getTypeDefByName: call convertAtlasTypeDefToOMTypeDef with abtd with name {}", abtd.getName());
            ret = convertAtlasTypeDefToOMTypeDef(userId, abtd);
        }
        catch (TypeErrorException e) {
...

    {
       ...
       if (name == null) {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                LOG.debug("{}: Atlas does not have the type with name {} ", methodName, name);
                // The AttributeTypeDef was not found - ensure abtd is null, exception to be handled below
                abtd = null;
            }
            else {
                LOG.error("{}: Caught exception trying to retrieve Atlas type with name {} ", methodName, name, e);
                LocalAtlasOMRSErrorCode errorCode = LocalAtlasOMRSErrorCode.REPOSITORY_ERROR;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(
                        this.getClass().getName(),
    ...
    public void addTypeDef(String  userId,
        ...
        if (fatalError || returnableTypeDef == null) {
            ...
            LOG.error("addTypeDef: could not initialise mapper or convert retrieved AtlasBaseTypeDef {} to OM TypeDef", atlasTypeName);
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
            ...
            UserNotAuthorizedException
    {
        ...
        if (superType != null) {
            ...
            if (FamousFive.omTypeRequiresSubstitution(superTypeName)) {
                LOG.debug("verifyTypeDef: supertype name {} requires substitution guid {}", superTypeName, superTypeGUID );
            ...
            if (existingSuperTypeDef == null) {
                ...
                if (typeNameToLookFor.equals(typeName))
                    LOG.debug("verifyTypeDef: no existing TypeDef for supertype with name {}", superTypeName);
                else
                    LOG.debug("verifyTypeDef: requested TypeDef for supertype name {} mapped to name {} which is not in the repo, ", superTypeName, typeNameToLookFor);
                // We cannot accept this typedef because it refers to a supertype that does not have a TypeDef in Atlas
                LOG.error("verifyTypeDef: The TypeDef with name {} and supertype name {} could not be modelled in Atlas", typeName, superTypeName);
                OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(typeName, typeDef.getGUID(), typeDefParameterName, methodName, repositoryName, typeDef.toString());
                throw new TypeDefNotSupportedException(
        ...
        switch (tdCat) {
            ...
            case ENTITY_DEF:
                ...
                if (FamousFive.omTypeRequiresSubstitution(typeName)) {
                    LOG.debug("verifyTypeDef: type name {} requires substitution", typeName);
                ...
                if (existingTypeDef == null) {
                    ...
                    if (entityTypeNameToLookFor.equals(typeName))
                        ...
                        LOG.debug("verifyTypeDef: requested TypeDef name {} mapped to name {} which is not in the repo, ", typeName, entityTypeNameToLookFor);
                    ret = false;
                }
            ...
            case RELATIONSHIP_DEF:
                ...
                for (RelationshipEndDef endDef : endDefs) {
                    ...
                    if (endDef == null || endDef.getEntityType() == null) {
                        ...
                        LOG.error("verifyTypeDef: The TypeDef with name {} has an invalid end definition {}", typeName, endDef);
                        OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
                        String errorMessage = errorCode.getErrorMessageId()
                                + errorCode.getFormattedErrorMessage(typeName, typeDef.getGUID(), typeDefParameterName, methodName, repositoryName, typeDef.toString());
                        throw new TypeDefNotSupportedException(
                    ...
                    if (FamousFive.omTypeRequiresSubstitution(entityTypeName)) {
                        LOG.debug("verifyTypeDef: supertype name {} requires substitution", entityTypeName);
                    ...
                    if (existingEntityTypeDef == null) {
                        ...
                        if (endTypeNameToLookFor.equals(entityTypeName))
                            LOG.debug("verifyTypeDef: no existing TypeDef for entity type with name {}", entityTypeName);
                        else
                            LOG.debug("verifyTypeDef: requested TypeDef for entity type name {} mapped to name {} which is not in the repo, ", entityTypeName, endTypeNameToLookFor);
                        // We cannot accept this typedef because it refers to an end entity type that does not have a TypeDef in Atlas
                        LOG.error("verifyTypeDef: The TypeDef with name {} and end entity type name {} could not be modelled in Atlas", typeName, entityTypeName);
                        OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
                        String errorMessage = errorCode.getErrorMessageId()
                                + errorCode.getFormattedErrorMessage(typeName, typeDef.getGUID(), typeDefParameterName, methodName, repositoryName, typeDef.toString());
                        throw new TypeDefNotSupportedException(
                ...
                if (validEntityDefs != null) {
                    ...
                    for (TypeDefLink validEntityDef : validEntityDefs) {
                        ...
                        if (FamousFive.omTypeRequiresSubstitution(validEntityTypeName)) {
                            LOG.debug("verifyTypeDef: supertype name {} requires substitution", validEntityTypeName);
                        ...
                        if (existingEntityTypeDef == null) {
                            ...
                            if (validTypeNameToLookFor.equals(validEntityTypeName))
                                LOG.debug("verifyTypeDef: no existing TypeDef for valid entity type with name {}", validEntityTypeName);
                            else
                                LOG.debug("verifyTypeDef: requested TypeDef for valid entity type name {} mapped to name {} which is not in the repo, ", validEntityTypeName, validTypeNameToLookFor);
                            // We cannot accept this typedef because it refers to a valid entity type that does not have a TypeDef in Atlas
                            LOG.error("verifyTypeDef: The TypeDef with name {} and valid entity type name {} could not be modelled in Atlas", typeName, validEntityTypeName);
                            OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
                            String errorMessage = errorCode.getErrorMessageId()
                                    + errorCode.getFormattedErrorMessage(typeName, typeDef.getGUID(), typeDefParameterName, methodName, repositoryName, typeDef.toString());
                            throw new TypeDefNotSupportedException(
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                LOG.debug("{}: Atlas does not have the type with name {} ", methodName, typeName);
                // The TypeDef was not found - ensure atlasPreTypeDef is null, exception to be handled below
                atlasPreTypeDef = null;

            } else {

                LOG.error("{}: Caught exception trying to retrieve Atlas type with name {} ", methodName, typeName, e);
                LocalAtlasOMRSErrorCode errorCode = LocalAtlasOMRSErrorCode.REPOSITORY_ERROR;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(
                        this.getClass().getName(),
    ...
    public boolean isEntityProxy(String guid) throws EntityNotKnownException {
        ...
        return (atlasEntity.isProxy() == null ? false : Boolean.valueOf(atlasEntity.isProxy()));
    }
...

    {
        ...
        // Sequenced queries are not yet supported. TODO commented block to be removed when function has been tested
        ...
        // Paged queries are not yet supported. TODO commented block to be removed when function has been tested
        //if (pageSize != 0) {
        ...
        // Result offset is not yet supported. TODO commented block to be removed when function has been tested
            ...
            UserNotAuthorizedException
    {
        ...
        LOG.debug("addEntity: typeGUID is {} maps to type with name {}", entityTypeGUID, typeDef.getName());
        ...
        try {
            ...
            // An OM EntityDef only has (at most)one superType - so retrieve it if it exists and wrap into a list of length one...
            ...
            /* Properties in Egeria may be optional or mandatory. We would expect all mandatory Egeria prperties to be supplied, as checked
             ...
             * a tigter constraint from the Atlas typesystem, synthesie a recognisable value for the attribute.
             ...
             * will ensure that every supertye is visited.
             */
            ...
            try {
                ...
                LOG.error("addEntity: Could not get attribute defs for type {}", atlasTypeName, e);
            ...
            while (attributeNamesIter.hasNext()) {
                ...
                if (attrDef != null && attrDef.getValuesMinCount() > 0 ) {
                    ...
                    if (suppliedProperties.getPropertyValue(key) == null) {
                        ...
                        switch (attrDef.getTypeName()) {
                            case "string":
                                ...
                                break;
                            default:
                                LOG.error("addEntity: need to synthesize attribute of type {}",attrDef.getTypeName());
                        }
                    }
                }
            }
            LOG.debug("addEntity: Supplementary attributes contains {}", supplementaryAttrs);
            ...
            try {
                ...
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(atlasEntity.toString(), "addEntity", metadataCollectionId, e.getMessage());
            ...
            if (emr != null) {
                ...
                if (atlasEntityHeaders == null || atlasEntityHeaders.size() != 1) {
                    ...
                    if (aeh.getStatus() != AtlasEntity.Status.ACTIVE) {
                        ...
                        AtlasEntity.AtlasEntityWithExtInfo retrievedAtlasEntity = null;
                        try {
                            retrievedAtlasEntity = entityStore.getById(newEntityGuid);
                        } catch (Exception e) {
                            LOG.error("addEntity:caught exception from retrieval of created entity {}", e);
                            retrievedAtlasEntity = null;
                        }
                        if (retrievedAtlasEntity != null) {
                            // Set the actual updated Time - it can differ from the created time because atlas will auto update the modification metadata.
                            entityDetail.setUpdateTime(retrievedAtlasEntity.getEntity().getUpdateTime());
                        } else {
                            LOG.error("addEntity: retrieval of created entity returned null");
    ...
    private Map<String,AtlasStructDef.AtlasAttributeDef>  getAllEntityAttributeDefs(String typeName) throws TypeErrorException {
        Set<String> visitedNames = new HashSet<>();
        return getAllEntityAttributeDefs(typeName,visitedNames);
    }

    // Utility: to get all the type names in the supertype hierarchy - including both sides of the F5 facade
    private Map<String,AtlasStructDef.AtlasAttributeDef>  getAllEntityAttributeDefs(String typeName, Set<String> visited) throws TypeErrorException {
        ...
        try {
            ...
            if (abtd instanceof AtlasEntityDef) {
                ...
                if (myAttrDefs != null && !myAttrDefs.isEmpty()) {
                    for (AtlasStructDef.AtlasAttributeDef myAttrDef : myAttrDefs) {
                        ...
                        LOG.debug("type {} contributes attribute {} ", typeName, attrDef.getName());
                ...
                if (!superTypeNames.isEmpty()) {
                    ...
                    while (superTypeNameIterator.hasNext()) {
                        ...
                        if (visited.contains(superTypeName)) {
                            continue; // skip this supertype, you have already visited it
                        } else {
                            ...
                            if (inheritedAttrDefs != null && !inheritedAttrDefs.isEmpty()) {
                                ...
                                while (mapIterator.hasNext()) {
                                    ...
                                    LOG.debug("type {} inherits from {} attribute name {} has cardinality {} isOptional {} valuesMinCount {} valuesMaxCount {}", typeName, superTypeName, attrName, cardinality, isOptional, valuesMinCount, valuesMaxCount);
                ...
                LOG.error("Found a typedef that is not an AtlasEntityDef ");
            ...
            LOG.error("getAllSupertypeNames - hit an exception {} ",e);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            // Use an import stream so that system attributes are updated
            ...
            UserNotAuthorizedException
    {
        ...
        LOG.debug("updateEntityProperties: current entity properties {} ", entity.getProperties());
        ...
        LOG.debug("updateEntityProperties: desired entity properties {} ", entity.getProperties());
        ...
        try {
            ...
            AtlasEntity atlasEntity = convertOMEntityDetailToAtlasEntity(userId, entity);
            LOG.debug("updateEntityProperties: atlasEntity to update is {}", atlasEntity);
            ...
            // Initial strategy is as follows:
            // If the properties passed to this method are not adequate to satisfy the mandatory
            InstanceProperties suppliedProperties = properties;
            Map<String, Object> supplementaryAttrs = new HashMap<>();
            Iterator<String> attributeNamesIter = allAttrDefs.keySet().iterator();
            while (attributeNamesIter.hasNext()) {
                String key = attributeNamesIter.next();
                ...
                if (suppliedProperties.getPropertyValue(key) == null) {
                    ...
                    // or we may need to synthesize it - if the corresponding Atlas attribute is mandatoru
                    ...
                    if (attrDef != null && attrDef.getValuesMinCount() > 0) {
                        ...
                        switch (attrDef.getTypeName()) {
                            ...
                            default:
                                LOG.error("updateEntityProperties: Mandatory attribute {} of type {} not supplied and cannot be syntesized", key, attrDef.getTypeName());
            ...
            LOG.debug("updateEntityProperties: will update using atlasEntity {} ", atlasEntity);

            // Construct an AtlasEntityWithExtInfo and call the repository
            AtlasEntity.AtlasEntityWithExtInfo atlasEntityToUpdate = new AtlasEntity.AtlasEntityWithExtInfo(atlasEntity);
            // Use an import stream so that system attributes are updated - we are bumping the version number
            AtlasEntityStreamForImport eis = new AtlasEntityStreamForImport(atlasEntityToUpdate, null);

            // EMR is deliberately ignored - instead the whole entity is read back from Atlas.
            EntityMutationResponse emr = entityStore.createOrUpdateForImport(eis);
            ...
            if (atlasEntWithExt == null) {
                LOG.error("updateEntityProperties: Could not find entity with guid {} ", entityGUID);
                OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
                String        errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(entityTypeGUID, entityGUIDParameterName, methodName, repositoryName);
            ...
            // Extract the entity
            AtlasEntity atlasEntityRetrieved = atlasEntWithExt.getEntity();
            LOG.debug("updateEntityProperties: returned atlasEntity is {}", atlasEntityRetrieved);
            ...
            LOG.debug("updateEntityProperties: returned entity mapped back to EntityDetail is {}", returnEntityDetail);
            ...
            LOG.error("updateEntityProperties: Caught exception from AtlasEntityMapper {}", e);
            ...
            LOG.error("updateEntityProperties: Caught exception from Atlas {}", e);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("deleteEntity: Caught exception from AtlasEntityMapper {}", e);
            ...
            UserNotAuthorizedException
    {
        ...
        // TODO - when ATLAS-2774 is implemented it should be possible to perform an entityStore hard delete (purge) regardless of the configured setting of delete handler ... but for now we need to check that Atlas is using a HARD delete and fail otherwise
        if (atlasDeleteConfiguration == AtlasDeleteOption.SOFT) {
            LOG.error("purgeEntity: Atlas is configured for soft delete", deletedEntityGUID);  // not introducing specific exception because this code should be tempporary pending ATLAS-2774
        ...
        // On the basis (established just above) that Atlas is using the Hard delete handler, perform a delete. TODO - this code will need to change when ATLAS-2774 is implemented.
        try {
            ...
            LOG.error("purgeEntity: Caught exception from Atlas entityStore trying to perform hard delete of entity {}", deletedEntityGUID, e);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("classifyEntity: retrieved entity from Atlas {}", atlasEntityRetrieved);
            ...
            LOG.error("classifyEntity: retrieved entity from Atlas and converted it to OM EntityDetail {}", returnEntityDetail);
            return returnEntityDetail;
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.debug("addRelationship: retrieved Atlas entity one {}", atlasEnt1);
            ...
            LOG.debug("addRelationship: retrieved Atlas entity two {}", atlasEnt2);
            ...
            LOG.debug("addRelationship: converted to atlasRelationship {}", atlasRelationship);
            ...
            LOG.debug("addRelationship: new relationship {}", omRelationship);
...

    {
        ...
        // Set end1
        LOG.debug("addRelationship: set end 1 of type {}",omRelationship.getEntityOneProxy().getType());
        ...
        if (FamousFive.omTypeRequiresSubstitution(end1TypeName)) {
            LOG.error("convertOMRelationshipToAtlasRelationship: mapping F5 name for end 1 from  {} to {} ",end1TypeName,FamousFive.getAtlasTypeName(end1TypeName, null));
        ...
        // Set end2
        LOG.debug("addRelationship: set end 2 of type {}",omRelationship.getEntityTwoProxy().getType());
        ...
        if (FamousFive.omTypeRequiresSubstitution(end2TypeName)) {
            LOG.error("convertOMRelationshipToAtlasRelationship: mapping F5 name for end 2 from {} to {} ",end2TypeName,FamousFive.getAtlasTypeName(end2TypeName,null));
        ...
        LOG.debug("addRelationship: converted relationship {} ", atlasRelationship);
            ...
            UserNotAuthorizedException
    {
         ...
         * this purgeRelationship call. In this case, check that the relationship is in DELETED state.
         *
         * This method does not use the repository's deleteById method - instead it uses the purgeById
         * method - which guarantees a permanent removal independent of how the repository is configured.
         */
...

    {
        ...
        try {
            AtlasEntity atlasEntity = convertOMEntityDetailToAtlasEntity(userId, entity);
            LOG.debug("reTypeEntity: atlasEntity to update is {}", atlasEntity);
            ...
            if (atlasEntWithExt == null) {
                LOG.error("reTypeEntity: Could not find entity with guid {} ", entityGUID);
                OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
                String        errorMessage = errorCode.getErrorMessageId()
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            AtlasEntity atlasEntity = convertOMEntityDetailToAtlasEntity(userId, entity);
            LOG.debug("reHomeEntity: atlasEntity to update is {}", atlasEntity);
            ...
            if (atlasEntWithExt == null) {
                LOG.error("reHomeEntity: Could not find entity with guid {} ", entityGUID);
                OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
                String        errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage(entityGUID, entityParameterName, methodName, repositoryName);
            ...
            LOG.error("reHomeEntity: Caught exception from AtlasEntityMapper {}", e);
            ...
            LOG.error("reHomeEntity: Caught exception from Atlas {}", e);
            OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(entityGUID, entityParameterName, methodName, repositoryName);
            ...
            for (AtlasEnumDef atlasEnumDef : atlasEnumDefs) {
                ...
                if (typeKnown) {
                    try {
                        ...
                        LOG.error("processAtlasEnumDefs: gave up on AtlasEnumDef {}", atlasTypeName, e);
                        // swallow the exception to proceed with the next type...
                    }
                }
                else {
                    // this type is not known to the RCM - abandon it...
                    LOG.debug("processAtlasEnumDefs: the type {} found in Atlas is not known in the ODPi type system", atlasTypeName);
            ...
            for (AtlasEntityDef atlasEntityDef : atlasEntityDefs) {
                ...
                if (typeKnown) {
                    ...
                    try {
                        ...
                        LOG.error("processAtlasEntityDefs: gave up on AtlasEntityDef {}", atlasTypeName, e);
                        // swallow the exception to proceed with the next type...
                    }
                }
                else {
                    // this type is not known to the RCM - abandon it...
                    LOG.debug("processAtlasEntityDefs: the type {} found in Atlas is not known in the ODPi type system", atlasTypeName);
                    // continue with the next type...
                }
            }
        }
            ...
            for (AtlasClassificationDef atlasClassificationDef : atlasClassificationDefs) {
                ...
                if (typeKnown) {
                    ...
                    try {
                        ...
                        LOG.error("processAtlasClassificationDefs: gave up on AtlasClassificationDef {}", atlasTypeName, e);
                        // swallow the exception to proceed with the next type...
                    }
                }
                else {
                    // this type is not known to the RCM - abandon it...
                    LOG.debug("processAtlasClassificationDefs: the type {} found in Atlas is not known in the ODPi type system", atlasTypeName);
                    // continue with the next type...
                }
            ...
            for (AtlasRelationshipDef atlasRelationshipDef : atlasRelationshipDefs) {
                ...
                if (typeKnown) {
                    try {
                        ...
                        LOG.error("processAtlasRelationshipDefs: gave up on AtlasRelationshipDef {}", atlasTypeName, e);
                        // swallow the exception to proceed with the next type...
                    }
                }
                else {
                    // this type is not known to the RCM - abandon it...
                    LOG.debug("processAtlasRelationshipDefs: the type {} found in Atlas is not known in the ODPi type system", atlasTypeName);
                    // continue with the next type...
                }
            ...
            TypeErrorException
    {
        ...
        if (existingAttributeTypeDef == null) {
            ...
            LOG.debug("processAtlasEnumDef: add OM EnumDef to typeDefsForAPIed - {}", omrsEnumDef);
            ...
            throws TypeErrorException, RepositoryErrorException
    {
        ...
        LOG.debug("processAtlasEntityDef: call the mapper with typeName {}", typeName);
        ...
        if (superTypeLink != null) {
            ...
            if (!superTypeKnown) {
                ...
                LOG.debug("processAtlasEntityDef: type name {} refers to a supertype {} that is not known in OM ", entityDef.getName(), superTypeLink.getName());
        ...
        // Add the OM typedef to TDBC - this will ultimately get it into the TypeDefGallery
        LOG.debug("processAtlasEntityDef: adding entityDef to typeDefsForAPI {}",entityDef);
            ...
            TypeDefNotKnownException
    {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                LOG.debug("_getTypeDefGUIDByAtlasTypeName: Atlas does not have the type with name {} ", atlasTypeName);
                // The AttributeTypeDef was not found - return null
                return null;

            } else {
                LOG.error("_getTypeDefGUIDByAtlasTypeName: Caught exception trying to retrieve Atlas type with name {} ", atlasTypeName, e);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("name", "_getTypeDefGUIDByAtlasTypeName", metadataCollectionId);

                throw new RepositoryErrorException(errorCode.getHTTPErrorCode(),
                        this.getClass().getName(),
                        "_getTypeDefGUIDByAtlasTypeName",
                        errorMessage,
                        errorCode.getSystemAction(),
                        errorCode.getUserAction());
            }
